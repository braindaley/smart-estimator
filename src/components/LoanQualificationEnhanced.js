...
/**
 * Step-based LoanQualification Component with multi-step form approach
 */
export default function LoanQualificationEnhanced({ userId, onComplete }) {
  ...
  useEffect(() => {
    const refreshStepStatus = () => {
      ...
      console.log('[LoanQualification] Step status loaded:', {
        ...
        coApplicantCompleted: !!currentStepStatus.co_applicant_check?.completed,
  ...
  // Check if phone verification is needed and show modal (for old version)
  useEffect(() => {
    if (showOldVersion && stepStatusLoaded && !stepStatus.phone_verification?.completed) {
      setShowPhoneModal(true);
    }
  }, [showOldVersion, stepStatusLoaded, stepStatus.phone_verification?.completed]);

  // Handle multi-step form completion
  const handleMultiStepComplete = (data) => {
    console.log('[LoanQualification] Multi-step form completed:', data);
  ...
  // Bank connection handler (for old version)
  ...
  // Credit check handler (for old version)
  ...
  // Phone verification handler (for old version)
      ...
      <div className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="text-center mt-8 pt-6 border-t border-border">
          <button
            ...
            className="text-sm text-muted-foreground hover:text-foreground underline transition-colors mr-4"
          >
